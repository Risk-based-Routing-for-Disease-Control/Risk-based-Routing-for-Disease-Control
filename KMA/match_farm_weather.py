"""농장(ML_전국_농장현황_1year.csv)마다 가장 가까운 관측소를 찾아
조사날짜-30일~조사날짜 구간의 일별 날씨를 그대로 붙인다 -> KMA/ML_농장_날씨.csv

농장 좌표 기준 최근접 관측소를 구한 뒤 그 관측소의 날씨를 매칭하므로,
조사날짜가 같아도 농장 위치(=최근접 관측소)가 다르면 서로 다른 날씨 시계열이 붙는다.
결과는 농장 1건당 최대 31일치 행으로 늘어나는 long format이다.
"""
import os
...
def main():
    ...
    farm["조사날짜"] = pd.to_datetime(farm["조사날짜"])
    ...
    farm["window_start"] = farm["조사날짜"] - pd.Timedelta(days=WINDOW_DAYS)
    farm = farm.reset_index().rename(columns={"index": "farm_id"})

    merged = farm.merge(
        weather, left_on="matched_STN", right_on="STN", how="left", suffixes=("", "_weather")
    )
    merged = merged[(merged["TM"] >= merged["window_start"]) & (merged["TM"] <= merged["조사날짜"])]
